...
class modelResourceCoapButler (name : String ) : CoapResource(name) {
	...
	companion object {
		...
		fun create( a: ActorBasic, name: String, port:Int = 5683){
			...
			resourceCoap = modelResourceCoapButler( name )
			server.add( resourceCoap );
			println("--------------------------------------------------")
			println("Coap Server  [$a:: coap://localhost:$port/$name]  started");	
			println("--------------------------------------------------")
			butlerResourceModelSupport.setCoapResource(resourceCoap)
			server.start();
 		}		
	}
	...
	override fun handlePUT(exchange: CoapExchange?) {
		try {
			...
			println("actor:: $actor __ $value")
				GlobalScope.launch{
					MsgUtil.sendMsg( "replyFridge", "replyFridge( $value )", sysUtil.getActor("butler_fridge_handler")!!)					
					delay(100)  //give the time to change the model
