...
import itunibo.butler.butlerResourceModelSupport
import it.unibo.kactor.sysUtil
import itunibo.fridge.fridgeResourceModelSupport

class modelResourceCoapFridge (name : String ) : CoapResource(name) {
	...
	companion object {
		...
		fun create( a: ActorBasic, name: String, port:Int = 5683){
			actor = a
			val server   = CoapServer(port);		//COAP SERVER
			resourceCoap = modelResourceCoapFridge( name )
			server.add( resourceCoap );
			println("--------------------------------------------------")
			println("Coap Server  [$a:: coap://localhost:$port/$name]  started");	
			println("--------------------------------------------------")
			fridgeResourceModelSupport.setCoapResource(resourceCoap)
			server.start();
 		}		
	}
...
//		curmodelval = actor.getCurSol("STATE").toString()
	...
	override fun handlePUT(exchange: CoapExchange?) {
		try {
			val value = exchange!!.getRequestText()//new String(payload, "UTF-8");
			println("actor:: $actor __ $value")
				GlobalScope.launch{
					MsgUtil.sendMsg( "msgFridge", "msgFridge( $value )", actor)	
					exchange.respond(CHANGED, "handlePUT:: $value")
				}	
			//}
